...
def extract_command_names(record: Dict[str, Any], model:str, prompt_name, iteration) -> List[str]:
    ...
    pattern = re.compile(
    r'^(?:'
    # ISET/VSET queries ONLY (no bare ISET1 / VSET1)
    r'ISET[12](?!\d)\?|'
    r'VSET[12](?!\d)\?|'
    # ISET/VSET set commands MUST include a numeric value (accept ":" or whitespace, optional)
    r'ISET[12](?!\d)(?:[:\s]\s*\d+(?:\.\d+)?)|'
    r'VSET[12](?!\d)(?:[:\s]\s*\d+(?:\.\d+)?)|'
    # readbacks
    r'IOUT[12](?!\d)\?|VOUT[12](?!\d)\?|'
    # output + status/idn
    r'OUT(?:\s*(?:[01]|ON|OFF))?(?!\w)|STATUS\?|\*IDN\?|'
    # save/recall
    r'RCL[1-5](?!\d)|SAV[1-5](?!\d)|'
    # protections/beep (allow glued or spaced ON/OFF/0/1)
    r'(?:OCP|OVP|BEEP)(?:\s*(?:[01]|ON|OFF))?(?!\w)'
    r')$',
    re.IGNORECASE
)
    commands = [] 
    ...
    for command in list(commands_iter):
        ...
        base_command = list(dict_command.keys())[0]
       
        for param in dict_command[base_command]: 
            #if base_command == "*IDN?":
            #    print(base_command + param, model, prompt_name, iteration)
            full_string += param 
            done += 1 
            if done != len(dict_command[base_command]) and pattern.match(base_command + param):
                valid.append(base_command + param)
                #print(base_command + param, len(dict_command[base_command]), done, model, prompt_name, iteration, dict_command[base_command], dict_command)
                 
            
        if bool(pattern.match(full_string)) and done == len(dict_command[base_command]):
            valid.append(full_string)
            normalized_cmd = normalize_command(list(dict_command.keys())[0], True)
            
            if normalized_cmd in base_commands_list:
                
                base_commands.add(normalized_cmd)
            done = 0

        #for param in dict_command[base_command]:
        #    if bool(pattern.match(base_command + param)):
        #        valid.a

        if 'VSET' not in base_command and 'ISET' not in base_command:
            for param in dict_command[base_command]:
                base_cmd_with_param = base_command + param
                if bool(pattern.match(base_cmd_with_param)):
                    normalized_cmd = normalize_command(base_cmd_with_param, True)
                    if normalized_cmd in base_commands_list:
                        base_commands.add(normalized_cmd)

                
        commands.append(full_string)  #if model == "codegemma:2b" and prompt_name == "prompt1":
...
def load_known_commands(csv_path: str = KNOWN_COMMANDS_CSV) -> Set[str]:
    ...
    try:
        with open(csv_path, newline='') as f:
            ...
            for row in reader:
                specs[row["command"].strip()] = {
                    "num_of_parameters": row["num_of_parameters"].strip().lower(),
                    "parameter_type": row["parameter_list"].split(" "),
                }
    ...
    for model in sorted(max_iteration_by_model.keys()):
        ...
        for iteration in range(1, 51):
            ...
            if (model, iteration) in records_by_model_iteration and iteration <= max_iteration:
                ...
                commands = extract_command_names(records[0], model, prompt_name, iteration)
                ...
                #if prompt_name == "prompt1" and model == "qwen3-coder:latest":
                    #print(commands[0], model, prompt_name, iteration)
